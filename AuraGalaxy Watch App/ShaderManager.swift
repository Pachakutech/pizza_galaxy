...
let glslGalaxyBlend = """
    void main() {
        ...
        vec2 wrapped_uv_1 = fract(uv);          // Wrap first texture
        vec2 wrapped_uv_2 = fract(uv + 0.5);    // Offset second for cross-blend

        vec4 color1 = texture2D(u_galaxy_1, wrapped_uv_1);
        vec4 color2 = texture2D(u_galaxy_2, wrapped_uv_2);

        // Smooth blending based on scroll position (your original logic)
        float blend_h = smoothstep(0.0, 0.2, mod(uv.x, 1.0));
        blend_h *= smoothstep(1.0, 0.8, mod(uv.x, 1.0));
        float blend_v = smoothstep(0.0, 0.2, mod(uv.y, 1.0));
        blend_v *= smoothstep(1.0, 0.8, mod(uv.y, 1.0));

        gl_FragColor = mix(color1, color2, (blend_h + blend_v) / 2.0);
    }
    """
